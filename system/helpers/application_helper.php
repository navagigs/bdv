...
if ( ! function_exists('pages'))
{
	function pages($halaman, $jmlhalaman, $url, $id=""){
		if ($halaman > 1){
			$previous = $halaman-1;
			echo "<input type='button' value='<<' style='cursor:pointer;' onclick=\"location.href='".site_url().$url."/1".$id."'\"> <input type='button' value='<' style='cursor:pointer;' onclick=\"location.href='".site_url().$url."/".$previous.$id."'\"> ";
		} else {
			echo "<font color='#ccc'><input type='button' value='<<' style='cursor:pointer;' disabled='disabled'> <input type='button' value='<' style='cursor:pointer;' disabled='disabled'></font>";
		}
		if ($halaman < $jmlhalaman){
			$next = $halaman+1;
			echo "&nbsp;<input type='button' value='>' style='cursor:pointer;' onclick=\"location.href='".site_url().$url."/".$next.$id."'\"> <input type='button' value='>>' style='cursor:pointer;' onclick=\"location.href='".site_url().$url."/".$jmlhalaman.$id."'\">";
		} else {
			echo "<font color='#ccc' ><input type='button' value='>' style='cursor:pointer;' disabled='disabled'> <input type='button' value='>>' style='cursor:pointer;' disabled='disabled'></font>";
		}
	}
